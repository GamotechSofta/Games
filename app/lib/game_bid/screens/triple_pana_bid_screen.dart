...
class _TriplePanaBidScreenState extends State<TriplePanaBidScreen> {
  ...
  void _mergeBid(String number, int pts) {
    ...
    if (i >= 0) {
      ...
      _bids.add(
        _Line(
          id: DateTime.now().microsecondsSinceEpoch + _bids.length,
          number: number,
          points: '$pts',
          session: _session,
        ),
  ...
  void _onSpecialFieldChanged() {
    setState(() {
      if (_appliedQuickUniform != null) {
        ...
        final allMatch = _tripleKeys.every(
          (k) => (int.tryParse(_specialCtrls[k]!.text.trim()) ?? 0) == target,
        );
  ...
  ({int count, int points}) _liveStats() {
    if (_bids.isNotEmpty) {
      final points = _bids.fold<int>(
        0,
        (a, b) => a + (int.tryParse(b.points) ?? 0),
      );
  ...
  Widget build(BuildContext context) {
    ...
    final border = GameBidUi.bulkMatrixCellBorder(context, radius: 8);
    final focusedBorder = GameBidUi.bulkMatrixCellBorder(
      context,
      hasPoints: true,
      radius: 8,
    );
    ...
    return GameBidLayout(
      ...
      body: ListView(
        ...
        children: [
          ...
          Text(
            'Triple panas (000 – 999)',
            style: GameBidUi.panelTitle(context).copyWith(fontSize: 13),
          ),
          ...
          GridView.builder(
            ...
            itemBuilder: (context, i) {
              ...
              return Row(
                ...
                children: [
                  GameBidUi.betNumberChip(context, label: numStr, extent: tile),
                    ...
                    child: GameBidUi.betPointsRectangleSlot(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          fillColor: GameBidUi.bulkMatrixCellFill(context),
                          filled: true,
