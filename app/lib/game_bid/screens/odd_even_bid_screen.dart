...
class _OddEvenBidScreenState extends State<OddEvenBidScreen> {
  ...
  void _addToList() {
    ...
    for (final d in digits) {
      ...
      if (pts > 0) {
        final idx = _bids.indexWhere(
          (b) => b.number == '$d' && b.session == _session,
        );
        if (idx >= 0) {
          ...
          _bids.add(
            _Line(
              id: DateTime.now().millisecondsSinceEpoch + d,
              number: '$d',
              points: '$pts',
              session: _session,
            ),
    ...
    if (!any) {
      _toast(
        'Enter points for at least one ${_sideIsOdd ? "odd" : "even"} digit',
      );
      return;
    }
    setState(() {});
  ...
  void _tryAutoAddCurrentSide() {
    final digits = _sideIsOdd ? _oddDigits : _evenDigits;
    final hasAny = digits.any(
      (d) => (int.tryParse(_ctrl[d]!.text.trim()) ?? 0) > 0,
    );
    if (!hasAny) return;
    _addToList();
  }
  ...
  ({int count, int points}) _liveStats() {
    ...
    for (final e in pendingByDigit.entries) {
      final idx = _bids.indexWhere(
        (b) => b.number == e.key && b.session == _session,
      );
      if (idx >= 0) {
        points += e.value;
      } else {
            ...
            children: [
              Expanded(
                child: OutlinedButton(
                  ...
                  style: GameBidUi.modeToggle(context, _sideIsOdd),
              ...
              child: Row(
                children: [
                  SizedBox(
                    ...
                    child: BetDialKey(
                      digit: d,
                      selected: (int.tryParse(_ctrl[d]!.text) ?? 0) > 0,
                      pointsBadge: int.tryParse(_ctrl[d]!.text) ?? 0,
                      onPressed: null,
                    ),
                  ),
