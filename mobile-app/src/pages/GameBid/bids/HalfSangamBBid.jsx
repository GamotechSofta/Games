...
// Half Sangam (B): Open Ank (1 digit) + Close Pana (3 digits)
const HalfSangamBBid = ({ market, title, scheduleForTomorrow, slotBetweenPanaAndAnk, bids: bidsProp, setBids: setBidsProp }) => {
    ...
    const handleAdd = () => {
        ...
        if (!/^[0-9]$/.test(openAnk)) { showWarning('Please enter Open Ank (0-9).'); return; }
        if (!isValidAnyPana(closePana)) { showWarning('Close Pana must be a valid Pana (Single/Double/Triple).'); return; }
    ...
    return (
        <BidLayout market={market} title={title} bidsCount={bids.length} totalPoints={totalPoints} showDateSession session={session} setSession={setSession} sessionOptionsOverride={['CLOSE']} lockSessionSelect displayDate={formatDateDisplay(selectedDate)} walletBalance={walletBefore} hideFooter onSubmit={openReview} slotBetweenDateSession={slotBetweenPanaAndAnk}>
            <View style={s.content}>
                {warning ? <View style={s.warnBox}><Text style={s.warnText}>{warning}</Text></View> : null}
                <View style={s.inputRow}>
                    <Text style={s.label}>{t('gameBid.enterOpenAnk')}:</Text>
                    <TextInput style={s.input} value={openAnk} onChangeText={(v) => setOpenAnk(sanitizeDigits(v, 1))} placeholder={t('gameBid.digitPlaceholder')} placeholderTextColor="rgba(255,255,255,0.85)" selectionColor="#f2c14e" cursorColor="#fff" maxLength={1} keyboardType="numeric" />
                </View>
                ...
                <View style={s.inputRow}>
                    ...
                    <TextInput style={[s.input, closePanaInvalid && s.inputInvalid]} value={closePana} onChangeText={(v) => { const next = sanitizeDigits(v, 3); setClosePana(next); setClosePanaInvalid(!!next && next.length === 3 && !isValidAnyPana(next)); }} placeholder={t('gameBid.pana')} placeholderTextColor="rgba(255,255,255,0.85)" selectionColor="#f2c14e" cursorColor="#fff" maxLength={3} keyboardType="numeric" />
                </View>
                <View style={s.inputRow}>
