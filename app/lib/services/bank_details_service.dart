...
class BankDetailsService {
  ...
  Future<({bool success, List<Map<String, dynamic>> data, String? message})> listAccounts() async {
    ...
    final userId = AuthService.storedUserId(await AuthService.instance.getStoredUser());
    if (h.isEmpty) {
      return (success: false, data: <Map<String, dynamic>>[], message: 'Please log in');
    }
    final uri = Uri.parse('$kApiBaseUrl/bank-details').replace(
      queryParameters: {
  ...
  Future<({bool success, String? message})> deleteAccount(String id) async {
    ...
    if (userId == null) return (success: false, message: 'Please log in');
    final uri = Uri.parse('$kApiBaseUrl/bank-details/$id');
    final res = await http.delete(uri, headers: h, body: jsonEncode({'userId': userId}));
    return _boolMessage(res);
  }
  ...
  Future<({bool success, String? message})> setDefault(String id) async {
    ...
    final res = await http.post(uri, headers: h, body: jsonEncode({'userId': userId}));
    return _boolMessage(res);
  }
}
