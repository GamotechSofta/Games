...
class _SpDpMotorStubScreenState extends State<SpDpMotorStubScreen> {
  ...
  List<String> _generateDp(String digitStr) {
    final digits =
        digitStr.replaceAll(RegExp(r'\D'), '').split('').toSet().toList()
          ..sort();
    ...
    return out.where(isValidDoublePana).toList()
  ...
  void _generate({bool silent = false}) {
    ...
    if (digits.length < 2) {
      if (!silent)
        _toast('Enter at least 3 digits for SP and 2 digits for DP.');
    ...
    final p = pts;
    final sp = digits.length >= 3
        ? generateSpMotorSinglePanas(digits)
        : <String>[];
  ...
  Future<void> _submit() async {
    final active = _rows
        ...
        .toList();
    if (active.isEmpty)
      return _toast('Add at least one combination with points.');
    ...
    await showBidReviewDialog(
      ...
      labelKey: widget.includeTripleCombinations
          ? 'SP/DP/T Motor'
          : 'SP/DP Motor',
    ...
    return GameBidLayout(
      ...
      body: ListView(
        ...
        children: [
          ...
          BidDigitKeypad(
            selectedDigits: _selectedDigits.map((d) => '$d').toSet(),
