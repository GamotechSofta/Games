...
class _ListBidScreenState extends State<ListBidScreen> {
  ...
  void _mergeAdd(String number, String points, String sessionLabel) {
    ...
    final i = _bids.indexWhere(
      (b) => b.number == number && b.sessionLabel == sessionLabel,
    );
    if (i >= 0) {
  ...
  /// Adds easy-mode line when number + points are both ready (mobile keyboard flow).
  void _tryAddEasyIfReady() {
    final n = _numCtrl.text.trim();
    final p = _ptsCtrl.text.trim();
    if (p.isEmpty || (int.tryParse(p) ?? 0) <= 0) return;
    if (n.length != widget.maxLength) return;
    if (!widget.validate(n)) return;
    _addEasy();
  }
  ...
  void _upsertBulkBidWithPoints(String key, int points) {
    final i = _bids.indexWhere(
      (b) => b.number == key && b.sessionLabel == _session,
    );
    if (points <= 0) {
  ...
  void _clearSelectedBulkCombinations() {
    ...
    setState(() {
      _bids.removeWhere(
        (b) =>
            b.sessionLabel == _session && _selectedBulkKeys.contains(b.number),
      );
          ...
          children: _quickPoints.map((p) {
            ...
            return GameBidUi.quickPointsChip(
              ...
              onSelected: (_) {
                ...
                if (widget.useBulkSpecialUi &&
                    identical(ctrl, _bulkSpecialPtsCtrl)) {
  ...
  void _materializePendingBeforeSubmit() {
    ...
    if (!widget.useBulkSpecialUi) {
      for (final e in _specialCtrls.entries) {
        ...
        if (pts > 0) {
          final i = _bids.indexWhere(
            (b) => b.number == e.key && b.sessionLabel == _session,
          );
          if (i >= 0) {
            _bids[i] = _BidLine(
              id: _bids[i].id,
      ...
      if (pts > 0 && _selectedBulkKeys.isNotEmpty) {
        final sorted = _selectedBulkKeys.toList()
        ...
        final lines = _bids
            .map(
              (b) => PlaceBetLine(
                ...
                betOn: b.sessionLabel.toUpperCase() == 'CLOSE'
                    ? 'close'
                    : 'open',
              ),
            )
  ...
  Widget build(BuildContext context) {
    ...
    final specialTile = GameBidUi.betTileExtentForColumns(
        ...
        children: [
          ...
          if (_easy || widget.specialKeys == null) ...[
            TextField(
              ...
              onSubmitted: (_) => _tryAddEasyIfReady(),
              decoration: GameBidUi.inputDecoration(
                context,
                labelText:
                    widget.easyNumberLabel ??
                    (widget.maxLength == 2
                        ? 'Number (00-99)'
                        : 'Number (${widget.maxLength} digits)'),
                hintText: widget.easyNumberLabel != null ? 'Pana' : 'Enter number',
                counterText: '',
              ),
            ),
            ...
            TextField(
              ...
              onSubmitted: (_) => _tryAddEasyIfReady(),
              onEditingComplete: _tryAddEasyIfReady,
              decoration: GameBidUi.inputDecoration(
                context,
                labelText: 'Points',
              ),
            ...
            Row(
              children: [
                Expanded(
                  child: Text(
                    'Step 2: Select pana combinations',
                    style: GameBidUi.panelTitle(context).copyWith(fontSize: 14),
                  ),
                ),
                TextButton(
                  onPressed: _selectedBulkKeys.isEmpty
                      ? null
                      : _clearSelectedBulkCombinations,
            ...
            GridView.builder(
              ...
              itemBuilder: (context, i) {
                ...
                return InkWell(
                  ...
                  child: DecoratedBox(
                    decoration: GameBidUi.numberChipTileDecoration(
                      context,
                      selected: selected,
                    ),
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: TextStyle(
                            color: GameBidUi.onSurfaceMuted(
                              context,
                              selected ? 0.95 : 0.65,
                            ),
                            fontWeight: FontWeight.w600,
            ...
            GameBidUi.glassListRow(
              ...
              child: Text(
                ...
                style: TextStyle(
                  fontWeight: FontWeight.w600,
                  color: GameBidUi.onSurfaceMuted(context, 0.88),
                ),
              ),
            ),
          ] else
                          ...
                          Expanded(
                            child: GameBidUi.betPointsRectangleSlot(
                              ...
                              child: TextField(
                                ...
                                decoration: GameBidUi.inlinePointsDecoration(
                                  context,
                                ).copyWith(hintText: 'pts'),
                              ),
                            ),
                          ),
                        ],
