...
const GameManagement = () => {
    ...
    const [savingUrlFor, setSavingUrlFor] = useState('');
    ...
    const loadGames = async () => {
        try {
            ...
            const drafts = {};
            list.forEach((g) => {
                drafts[g.gameId] = g.launchBaseUrl || '';
            });
            setUrlDrafts(drafts);
        } catch (e) {
    ...
    const handleSaveLaunchUrl = async (game) => {
        setError('');
        setMsg('');
        try {
            setSavingUrlFor(game.gameId);
            const res = await adminFetch(`${API_BASE_URL}/admin/game/update`, {
                method: 'PUT',
                body: JSON.stringify({
                    gameId: game.gameId,
                    launchBaseUrl: (urlDrafts[game.gameId] || '').trim(),
                }),
            });
            const data = await res.json();
            if (!data.success) throw new Error(data.message || 'Failed to update launch URL');
            setMsg(`Launch URL saved for "${game.name}"`);
            await loadGames();
        } catch (e) {
            setError(e.message || 'Failed to update launch URL');
        } finally {
            setSavingUrlFor('');
        }
    };
        ...
        <AdminLayout onLogout={handleLogout} title="Game Management">
            <div className="w-full min-w-0 px-2 sm:px-3 md:px-4 pb-6">
                ...
                <div className="rounded-xl border border-gray-700 bg-gray-800/80 p-4 mb-5">
                    ...
                    <form onSubmit={handleAddGame} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        <input
                        ...
                        />
                        <input
                            className="sm:col-span-2 px-3 py-2.5 rounded-lg bg-gray-700 border border-gray-600 text-white"
                            placeholder="Launch URL (Render frontend, e.g. https://ludo-xxx.onrender.com/)"
                            value={form.launchBaseUrl}
                            onChange={(e) => setForm((p) => ({ ...p, launchBaseUrl: e.target.value }))}
                        />
                        <select
                            className="px-3 py-2.5 rounded-lg bg-gray-700 border border-gray-600 text-white"
                            value={form.status}
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="sm:col-span-2 lg:col-span-3 px-4 py-2.5 rounded-lg bg-yellow-500 hover:bg-yellow-600 text-black font-semibold disabled:opacity-50"
                        >
                        ...
                        <table className="w-full text-sm text-left">
                            <thead className="text-gray-300 border-b border-gray-700">
                                <tr>
                                    ...
                                    <th className="py-2 pr-3 min-w-[220px]">Launch URL</th>
                            ...
                            <tbody>
                                {games.map((game) => (
                                    <tr key={game._id || game.gameId} className="border-b border-gray-700/50 text-white align-top">
                                        ...
                                        <td className="py-2 pr-3">
                                            <div className="flex flex-col gap-2 min-w-[200px]">
                                                <input
                                                    className="w-full px-2 py-1.5 rounded bg-gray-700 border border-gray-600 text-white text-xs"
                                                    placeholder="https://your-game.onrender.com/"
                                                    value={urlDrafts[game.gameId] ?? ''}
                                                    onChange={(e) =>
                                                        setUrlDrafts((p) => ({ ...p, [game.gameId]: e.target.value }))
                                                    }
                                                />
                                                <button
                                                    type="button"
                                                    onClick={() => handleSaveLaunchUrl(game)}
                                                    disabled={savingUrlFor === game.gameId}
                                                    className="self-start px-2 py-1 rounded bg-yellow-500/90 hover:bg-yellow-500 text-black text-xs font-semibold disabled:opacity-50"
                                                >
                                                    {savingUrlFor === game.gameId ? 'Saving...' : 'Save URL'}
                                                </button>
                                            </div>
                                        </td>
                                        <td className="py-2 pr-3">
