...
import '../dp_common_generator.dart';
import '../game_bid_layout.dart';
import '../bid_digit_keypad.dart';
import '../game_bid_ui.dart';

class DpCommonBidScreen extends StatefulWidget {
  const DpCommonBidScreen({
    super.key,
  ...
  @override
  State<DpCommonBidScreen> createState() => _DpCommonBidScreenState();
}

class _DpCommonBidScreenState extends State<DpCommonBidScreen> {
  ...
  final _digitDisplayCtrl = TextEditingController();
  ...
  void _toggleDigit(int d) {
    ...
    setState(() {
      ...
      _digitDisplayCtrl.text = _selectedDigits.join(',');
    });
    _scheduleGenerate();
  }
  ...
  void _generate({bool silent = false}) {
    ...
    for (final d in _selectedDigits) {
      final r = generateDPCommon(digit: d, points: pts);
      if (!r.success) {
    ...
    final panas = merged.keys.toList()
  ...
  Future<void> _submit() async {
    final active = _rows
        .where((r) => (int.tryParse(r.points) ?? 0) > 0)
        .toList();
    if (active.isEmpty) {
    ...
    return GameBidLayout(
      ...
      body: ListView(
        ...
        children: [
          ...
          Text('Select Digits', style: GameBidUi.sectionLabel(context)),
          const SizedBox(height: 8),
          BidDigitKeypad(
            selectedDigits: _selectedDigits.toSet(),
            onDigitClick: _toggleDigit,
          ),
          const SizedBox(height: 8),
          TextField(
            readOnly: true,
            controller: _digitDisplayCtrl,
            decoration: GameBidUi.inputDecoration(
              context,
              labelText: 'Selected digits',
              hintText: 'Tap digits above',
            ),
          ),
          ...
          if (_rows.isEmpty)
            Text('No bets yet', style: GameBidUi.emptyHint(context))
          else
            ..._rows.map(
              (r) => ListTile(
                ...
                trailing: Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      onPressed: () => setState(
                        () => _rows = _rows.where((e) => e.id != r.id).toList(),
                      ),
                      icon: Icon(
                        Icons.close,
