...
/** Same status logic as MarketList: open/closed - King has openingNumber & closingNumber */
const getSlotStatus = (m, nowMs) => {
    const hasOpen = m.openingNumber && /^\d{3}$/.test(String(m.openingNumber));
    const hasClose = m.closingNumber && /^\d{3}$/.test(String(m.closingNumber));
    if (hasOpen && hasClose) return { status: 'closed', color: 'bg-red-600' };
    if (hasOpen) return { status: 'closed', color: 'bg-green-600' };
...
const KingBazaarManagement = ({ embedded = false }) => {
    ...
    // When coming from MarketDetail "Back to King Bazaar", open that market's tab
    useEffect(() => {
        const key = (location.state?.kingBazaarMarketKey || '').toString().trim().toLowerCase();
        if (!key || !kingBazaarGroups.length) return;
        if (kingBazaarGroups.some((g) => (g.key || '').toLowerCase() === key)) {
            setActiveTab(key);
        }
    ...
    const activeGroup = useMemo(() => kingBazaarGroups.find((g) => g.key === activeTab) || kingBazaarGroups[0] || null, [kingBazaarGroups, activeTab]);
    ...
    const slotsForTab = useMemo(() => {
        ...
        const list = (markets || []).filter((m) => {
            if (m.marketType !== 'king') return false;
    ...
    const handleDeleteGroup = async (pwd = '') => {
        ...
        try {
            ...
            if (data.success) {
                ...
                await fetchKingBazaarGroups();
                await fetchMarkets();
                if (activeTab === deleteGroupKey && kingBazaarGroups.length > 1) {
                    const next = kingBazaarGroups.find((g) => g.key !== deleteGroupKey);
    ...
    const handleAddSlotSubmit = async (e) => {
        ...
        try {
            ...
            const marketName = `${(activeGroup?.label || activeTab) || 'King Bazaar'} ${toDisplayTime(addTime.hour12, addTime.minute, addTime.ampm)}`;
            const payload = {
                marketName,
                startingTime: time24,
                closingTime: time24,
                marketType: 'king',
                kingBazaarGroup: activeTab,
            };
    ...
    return (
        <AdminLayout onLogout={handleLogout} title="King Bazaar">
            {content}
        </AdminLayout>
    );
};

export default KingBazaarManagement;
