...
const DepositWithdrawalHistory = () => {
    ...
    return (
        <AdminLayout onLogout={handleLogout} title={isDeposit ? 'Deposit History' : 'Withdrawal History'}>
            <div className="mb-6">
                ...
                <div className="flex flex-wrap items-center gap-3 sm:gap-4">
                    <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white flex items-center gap-2 sm:gap-3 shrink-0">
                        <span className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center shrink-0 ${isDeposit ? 'bg-emerald-500/20' : 'bg-red-500/20'}`}>
                            {isDeposit ? <FaArrowDown className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-400" /> : <FaArrowUp className="w-4 h-4 sm:w-5 sm:h-5 text-red-400" />}
                        </span>
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-sm font-semibold border whitespace-nowrap ${pageTab === 'deposit' ? 'bg-emerald-600 text-white border-emerald-500' : 'bg-gray-800 text-gray-300 border-gray-600 hover:border-gray-500'}`}
                        >
                            Deposits
                        </button>
                        <button
                            ...
                            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-sm font-semibold border whitespace-nowrap ${pageTab === 'withdrawal' ? 'bg-red-600 text-white border-red-500' : 'bg-gray-800 text-gray-300 border-gray-600 hover:border-gray-500'}`}
                        >
                            Withdrawals
                        </button>
                    </div>
            ...
            <div className="bg-gray-800/60 rounded-xl p-3 sm:p-4 border border-gray-700 mb-4">
                <div className="flex flex-nowrap items-center gap-1.5 sm:gap-2 overflow-x-auto pb-0.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    <span className="text-xs sm:text-sm font-semibold text-gray-300 shrink-0 pr-1 sm:pr-2 whitespace-nowrap uppercase tracking-wider">Date range</span>
                    {PRESETS.map((p) => (
                        <button
                            key={p.id}
                            type="button"
                            onClick={() => handlePreset(p.id)}
                            className={`px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium shrink-0 whitespace-nowrap transition-colors ${!customMode && datePreset === p.id ? 'bg-orange-500 text-white' : 'bg-gray-700 text-white hover:bg-gray-600'}`}
                        >
                            {p.label}
                        </button>
                    ))}
                    <span className="text-xs text-gray-400 shrink-0 pl-1">From</span>
                    <input
                        type="date"
                        value={customFrom}
                        onChange={(e) => setCustomFrom(e.target.value)}
                        className="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg bg-gray-700 border border-gray-600 text-xs sm:text-sm text-white shrink-0"
                    />
                    ...
                    <button
                        ...
                        className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg bg-amber-500 text-black font-semibold text-xs sm:text-sm shrink-0 whitespace-nowrap"
                    >
                        Apply
                    </button>
                </div>
