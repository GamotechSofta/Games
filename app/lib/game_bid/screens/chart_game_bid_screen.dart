...
class _ChartGameBidScreenState extends State<ChartGameBidScreen> {
  ...
  List<({String id, String chart, String pana, String label, String points})>
  _rows = [];
  Timer? _addDebounce;
    ...
    setState(() {
      ...
      for (final pana in numbers) {
        final idx = next.indexWhere(
          (r) => r.chart == _selectedChart && r.pana == pana,
        );
        if (idx >= 0) {
          final old = next[idx];
    ...
    return GameBidLayout(
      ...
      body: ListView(
        ...
        children: [
          ...
          Text('Select Chart', style: GameBidUi.sectionLabel(context)),
          ...
          Wrap(
            ...
            children: _chartData.keys.map((chart) {
              ...
              return OutlinedButton(
                ...
                style: GameBidUi.outlineDigit(context, sel, extent: tile),
                child: Text(
                  chart,
          ...
          const SizedBox(height: 12),
          Text('Select Digit', style: GameBidUi.sectionLabel(context)),
          const SizedBox(height: 8),
          BidDigitKeypad(
            digits: _digitOrder,
            selectedDigits: _selectedDigit != null && _selectedDigit!.isNotEmpty
                ? {_selectedDigit!}
                : const {},
            onDigitClick: (d) {
