...
bool _isFundsRoute(String path) =>
    path == '/funds' || path.startsWith('/funds/');
...
bool shellShowsMobileDrawer(String path) =>
    path == '/' || shellShowsMobileBottomNav(path);
...
class _MainShellState extends State<MainShell> {
  ...
  Widget build(BuildContext context) {
    ...
    final fullScreenContent = false;
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final casinoShell = shellUsesCasinoStyle(widget.path) && isDark;
    final showBottomNav = width < 768 && shellShowsMobileBottomNav(widget.path);
    final showDrawer = shellShowsMobileDrawer(widget.path);
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        endDrawer: showDrawer
            ? AppDrawer(shellContext: context, currentPath: widget.path)
            : null,
        body: _HomeBody(
