...
// Half Sangam (A): Open Pana (3 digits) + Close Ank (1 digit)
const HalfSangamABid = ({ market, title, scheduleForTomorrow, slotBetweenPanaAndAnk, bids: bidsProp, setBids: setBidsProp }) => {
    ...
    const handleAdd = () => {
        ...
        if (!/^[0-9]$/.test(closeAnk)) { showWarning('Please enter Close Ank (0-9).'); return; }
    ...
    return (
        <BidLayout market={market} title={title} bidsCount={bids.length} totalPoints={totalPoints} showDateSession session={session} setSession={setSession} sessionOptionsOverride={['OPEN']} lockSessionSelect displayDate={formatDateDisplay(selectedDate)} walletBalance={walletBefore} hideFooter onSubmit={openReview} slotBetweenDateSession={slotBetweenPanaAndAnk}>
            <View style={s.content}>
                ...
                <View style={s.inputRow}>
                    ...
                    <TextInput style={s.input} value={closeAnk} onChangeText={(v) => setCloseAnk(sanitizeDigits(v, 1))} placeholder="0-9" placeholderTextColor="#6b7280" maxLength={1} />
                </View>
                <View style={s.inputRow}>
