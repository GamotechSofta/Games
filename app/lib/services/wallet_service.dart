...
class WalletService {
  ...
  Future<Map<String, String>> _headers() async {
    ...
    final t = AuthService.sessionToken(u);
  ...
  Future<({bool success, num? balance, String? message})> fetchBalance() async {
    ...
    final uri = Uri.parse('$kApiBaseUrl/wallet/balance').replace(
      queryParameters: {
  ...
  Future<({bool success, List<Map<String, dynamic>> data, String? message})> fetchMyTransactions({
    ...
    final h = await _headers();
    final uri = Uri.parse('$kApiBaseUrl/wallet/my-transactions').replace(
      queryParameters: {
        if (userId != null) 'userId': userId,
        'limit': '$limit',
        'includeBet': includeBet ? '1' : '0',
      },
    );
    final res = await http.get(uri, headers: h);
