...
const StarlineManagement = ({ embedded = false }) => {
    ...
    const handleAddMarketSubmit = async (e) => {
        ...
        setAddMarketError('');
        try {
            const res = await adminFetch(`${API_BASE_URL}/markets/starline-groups`, {
                method: 'POST',
    ...
    const handleDeleteGroup = async (pwd = '') => {
        ...
        try {
            ...
            const res = await adminFetch(`${API_BASE_URL}/markets/starline-groups/${encodeURIComponent(deleteGroupKey)}`, opts);
            const data = await res.json();
            if (data.success) {
    ...
    const handleCheckOpen = async () => {
        ...
        setPreview(null);
        try {
            const res = await adminFetch(`${API_BASE_URL}/markets/preview-declare-open/${id}?openingNumber=${encodeURIComponent(val)}`);
            const data = await res.json();
    ...
    const performDeclareOpen = async (pwd) => {
        ...
        try {
            ...
            const res = await adminFetch(`${API_BASE_URL}/markets/declare-open/${selectedResultMarket._id}`, {
                method: 'POST',
                body: JSON.stringify(body),
            });
            const data = await res.json();
    ...
    const handleClearResult = async () => {
        ...
        try {
            const res = await adminFetch(`${API_BASE_URL}/markets/clear-result/${selectedResultMarket._id}`, { method: 'POST' });
            const data = await res.json();
            if (data.success) {
                            ...
                            <div className={COMPACT_MARKET_GRID_CLASS}>
                                {[...starlineGroups]
                                    ...
                                    .map((g) => {
                                        ...
                                        return (
                                            <div key={g.key} className={COMPACT_MARKET_CARD_CLASS}>
                                                <div className="flex items-center gap-1.5 mb-1 min-w-0">
                                                    <span className={`${statusColor} ${COMPACT_MARKET_STATUS_CLASS}`}>
                                                        {statusLabel}
                                                    </span>
                                                    <h3 className="text-xs sm:text-sm font-bold text-white truncate flex-1 min-w-0" title={g.label}>
                                                        {g.label}
                                                    </h3>
                                                </div>
                                                <p className="text-amber-400 font-mono text-[11px] sm:text-xs tracking-wide truncate mb-2">
                                                    {slotCount} slot{slotCount !== 1 ? 's' : ''}
                                                </p>
                                                <div className="flex flex-col gap-0.5 text-[10px] sm:text-[11px] text-gray-400 mb-2 flex-1 min-w-0">
                                                    {slotCount > 0 && openCount > 0 && (
                                                ...
                                                <div className="grid grid-cols-2 gap-1 mt-auto pt-1 border-t border-gray-700/60">
                                                    <button
                                                        ...
                                                        className={`${COMPACT_MARKET_ACTION_BTN_CLASS} bg-amber-600 hover:bg-amber-500 text-black`}
                                                    >
                                                        Manage
                                                    </button>
                                                    <button
                                                        ...
                                                        className={`${COMPACT_MARKET_ACTION_BTN_CLASS} bg-red-600/90 hover:bg-red-500 text-white`}
                                                    >
                                                        Delete
                                                    </button>
                                                </div>
