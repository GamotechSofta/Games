...
class TriplePanaBulkBidScreen extends StatefulWidget {
  ...
  @override
  State<TriplePanaBulkBidScreen> createState() =>
      _TriplePanaBulkBidScreenState();
}
...
class _TriplePanaBulkBidScreenState extends State<TriplePanaBulkBidScreen> {
  ...
  bool _isValidTriple(String s) =>
      RegExp(r'^\d{3}$').hasMatch(s) && s[0] == s[1] && s[1] == s[2];
  ...
  void _handleNumberChange(String value) {
    ...
    final raw = value
        ...
        .substring(0, value.replaceAll(RegExp(r'\D'), '').length.clamp(0, 3));
    String next = '';
  ...
  void _addToList() {
    ...
    if (!_isValidTriple(n))
      return _toast('Invalid triple pana. Use 000, 111, 222 ... 999.');
    setState(() {
      _mergeAdd(n, pts);
      _numCtrl.clear();
      _ptsCtrl.clear();
    ...
    return GameBidLayout(
      ...
      body: ListView(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: _statCard('Count', '${stats.count}')),
              const SizedBox(width: 8),
              Expanded(child: _statCard('Bet Amount', '${stats.amount}')),
            ],
          ),
      ...
      child: Column(
        children: [
          Text(
            ...
            style: TextStyle(
              fontSize: 11,
              color: GameBidUi.onSurfaceMuted(context, 0.65),
              fontWeight: FontWeight.w600,
            ),
          ),
