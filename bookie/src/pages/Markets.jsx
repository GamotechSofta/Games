    ...
    const renderMarketCard = (market) => {
        ...
        if (marketType === 'main') {
            ...
            return (
                <div key={market._id} className="glass-panel rounded-2xl p-4 relative overflow-hidden group hover:shadow-[0_0_30px_-10px_rgba(245,158,11,0.15)] transition-all duration-300 border border-white/5 hover:border-amber-500/30">
                    ...
                    <div className="flex justify-between items-center mb-3 text-xs font-medium text-slate-400 bg-black/20 rounded-lg p-2 border border-white/5">
                        <div className="text-center">
                            <span className="block text-[9px] uppercase tracking-wider mb-0.5 opacity-70">Open</span>
                            <span className="text-white font-mono text-xs">{market.startingTime}</span>
                        </div>
                        <div className="h-4 w-px bg-white/10"></div>
                        <div className="text-center">
                            <span className="block text-[9px] uppercase tracking-wider mb-0.5 opacity-70">Close</span>
                            <span className="text-white font-mono text-xs">{market.closingTime}</span>
                        </div>
                    </div>

                    <div className="mb-2 text-center">
                        <div className="text-[9px] font-bold text-amber-500 uppercase tracking-[0.15em] mb-1.5 opacity-80">Result</div>
                        <div className="bg-black/40 border border-white/10 rounded-xl py-2.5 px-2 relative overflow-hidden group-hover:border-amber-500/30 transition-colors">
                            ...
                            <div className="relative z-10 flex items-center justify-center gap-1.5 font-mono font-bold text-xl text-white tracking-widest">
                                <span className={openPanna.includes('*') ? 'text-slate-600' : 'text-amber-400'}>{openPanna}</span>
                                <span className="text-slate-600 text-sm">-</span>
                                <span className={jodi.includes('*') ? 'text-slate-600' : 'text-white'}>{jodi}</span>
                                <span className="text-slate-600 text-sm">-</span>
        ...
        return (
            <div key={market._id} className="glass-panel rounded-2xl p-4 relative overflow-hidden group hover:shadow-[0_0_30px_-10px_rgba(245,158,11,0.15)] transition-all duration-300 border border-white/5 hover:border-amber-500/30">
                ...
                <div className="flex justify-center items-center mb-3 text-xs font-medium text-slate-400 bg-black/20 rounded-lg p-2 border border-white/5">
                    <div className="text-center">
                        <span className="block text-[9px] uppercase tracking-wider mb-0.5 opacity-70">Closing Time</span>
                        <span className="text-white font-mono text-xs">{market.closingTime || market.startingTime}</span>
                    </div>
                </div>
            ...
            <div className="max-w-[1600px] mx-auto">
                <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-slate-400 text-sm mt-1">View live market results and timings</p>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="glass-panel rounded-2xl p-12 text-center border border-white/5">
                        ...
                        <h3 className="text-white font-bold text-lg mb-2">No Markets Found</h3>
