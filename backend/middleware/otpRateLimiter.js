...
const OTP_SEND_WINDOW_MS = toPositiveInt(process.env.OTP_SEND_WINDOW_MS, 10 * 60 * 1000); // 10 minutes
const OTP_SEND_MAX = toPositiveInt(process.env.OTP_SEND_MAX, 10);
const OTP_VERIFY_WINDOW_MS = toPositiveInt(process.env.OTP_VERIFY_WINDOW_MS, 10 * 60 * 1000); // 10 minutes
const OTP_VERIFY_MAX = toPositiveInt(process.env.OTP_VERIFY_MAX, 20);

const buildOtpKey = (req) => {
    const phone = normalizePhone(req?.body?.phone);
    const ip = req.ip || req.headers['x-forwarded-for'] || 'unknown-ip';
    // Key by phone + IP so one shared IP does not throttle all users together.
